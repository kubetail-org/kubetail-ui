...
const FormControl = React.forwardRef(({ as = 'input', className, id, ...props }, ref) => {
    const Tag = as;
    const { controlId } = useContext(FormGroupContext);
    return (React.createElement(Tag, { ...props, ref: ref, id: id || controlId, className: cn(baseCls, className) }));
});
